...
P3:
    CMP R1, 1       ; se estado = 1
    JZ  P3_1
    CMP R1, 2       ; se estado = 2
    JZ  P3_2
sai_P3:
    RET             ; sai do processo. Dá oportunidade aos outros
                    ; processos de se executarem
...
P3_1:
    MOVB R0, [R9]   ; lê porto de entrada
    BIT  R0, 0      ; testa bit 0 do R0
    JNZ   sai_P3     ; se RTC=1 espera que passe para 0
    MOV  R1, 2      ; passa ao estado 2 do P3
    CMP  R7, ON     ; vê se a contagem do display está permitida
    JNZ  sai_P3     ; se não estiver, sai sem fazer nada
    SUB  R8, 2      ; incrementa contador
    MOVB [R10], R8  ; atualiza display
    JMP  sai_P3 	
...
P3_2:
    ...
    BIT  R0, 0      ; testa bit 0 do R0
    JZ  sai_P2     ; se RTC=0 espera que passe para 1
    MOV  R1, 1      ; passa ao estado 1 do P2
    JMP  sai_P2	
