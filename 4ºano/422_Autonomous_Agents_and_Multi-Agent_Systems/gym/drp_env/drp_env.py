...
class DrpEnv(gym.Env):
	...
	def dijkstra(self, graph, start):
		...
		# Set of vertices whose shortest distance from start is already calculated
		visited = set()
	...
	def get_actions_to_goal(self, current_node:int, goal_node:int, inaccessible_node:int = -1):
		edges = {}
		for i, j in self.ee_env.edge_labels:
			cost = self.ee_env.edge_labels[(i, j)]
			# paths.setdefault(i, {})[j] = cost
			if i not in edges.keys():
				edges[i] = {}
			edges[i][j] = cost
			# paths.setdefault(j, {})[i] = cost
			if j != inaccessible_node:
				if j not in edges.keys():
					edges[j] = {}
				edges[j][i] = cost
		print(self.dijkstra(edges, current_node)[1])
		input()
		return self.dijkstra(edges, current_node)[1][goal_node]
