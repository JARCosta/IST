...
from agents.PfrlAgent import PfrlAgent
from agents.RandomAgent import RandomAgent
from drp_env.drp_env import DrpEnv

def policy(obs, env:DrpEnv):

    agent = PfrlAgent(0, env) # only working with pfrl agents, for now
    
    actions = []
    for agent_id in range(env.n_agents):
        if agent_id < 3:
            agent.load(f"./models/sample_model{agent_id}")
            action = agent.act(obs[agent_id])
            actions.append(action)
    return actions

def policy_evaluation(drone_num, map_name, reward_list, start, goal, render):
