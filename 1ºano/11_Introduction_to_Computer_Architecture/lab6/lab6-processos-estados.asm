; *********************************************************************************
; * IST-UL
; * Modulo:    lab6-processos-estados.asm
; * Descrição: Este programa ilustra o funcionamento de vários processos cooperativos,
; *            incluindo um com diversos estados.
; *            Igual ao lab6-processos-teclado-OK.asm, mas agora com o processo
; *            anima_displays com 4 estados.
; *
; *********************************************************************************
...
AUMENTA_DISPLAYS    EQU  0    ; Estado do processo anima_displays
VAI_DIMINUIR        EQU  1    ; Estado do processo anima_displays
DIMINUI_DISPLAYS    EQU  2    ; Estado do processo anima_displays
VAI_AUMENTAR        EQU  3    ; Estado do processo anima_displays

NENHUMA_TECLA       EQU  0    ; valor lido das colunas do teclado sem tecla carregada
TECLA_C             EQU  1    ; coluna no teclado da tecla C
TECLA_D             EQU  2    ; coluna no teclado da tecla D
...
estado_anima_displays:             ; variável de estado do processo anima_displays
          WORD AUMENTA_DISPLAYS    ; estado inicial, por omissão
...
ciclo:
     ...
     CALL anima_displays      ; trata dos displays
...
; **********************************************************************
; ANIMA_DISPLAYS - Processo que trata do valor dos displays.
;                  A tecla C faz o display aumentar; a tecla D faz diminiur.
; **********************************************************************
anima_displays:
     PUSH R0
     PUSH R1
...
; Executa estado AUMENTA_DISPLAYS
     MOV  R4, 1                         ; aumenta displays de 1 unidade
     CALL altera_displays
     MOV  R3, coluna_carregada
     MOV  R1, [R3]                      ; obtém em R1 a informação sobre se há ou não tecla premida
     MOV  R2, TECLA_D                   ; coluna da tecla D
     CMP  R1, R2                        ; a tecla D premida?
     JNZ  sai_anima_displays            ; se não, mantém o estado
     MOV  R1, VAI_DIMINUIR              ; se sim, muda para o próximo estado
     MOV  [R0], R1                      ; atualiza a variável de estado 
     JMP  sai_anima_displays            ; na próxima execução já irá para o novo estado
...
; Executa estado VAI_DIMINUIR
     MOV  R3, coluna_carregada
     MOV  R1, [R3]                      ; obtém em R1 a informação sobre se há ou não tecla premida
     MOV  R2, NENHUMA_TECLA
     CMP  R1, R2                        ; nenhuma tecla premida?
     JNZ  sai_anima_displays            ; se ainda houver uma tecla premida, mantém o estado
     MOV  R1, DIMINUI_DISPLAYS          ; se sim, muda para o próximo estado
     MOV  [R0], R1                      ; atualiza a variável de estado 
     JMP  sai_anima_displays            ; na próxima execução já irá para o novo estado
...
; Executa estado DIMINUI_DISPLAYS
     MOV  R4, -1                        ; diminui displays de 1 unidade
     CALL altera_displays
     MOV  R3, coluna_carregada
     MOV  R1, [R3]                      ; obtém em R1 a informação sobre se há ou não tecla premida
     MOV  R2, TECLA_C                   ; coluna da tecla C
     CMP  R1, R2                        ; a tecla C premida?
     JNZ  sai_anima_displays            ; se não, mantém o estado
     MOV  R1, VAI_AUMENTAR              ; se sim, muda para o próximo estado
     MOV  [R0], R1                      ; atualiza a variável de estado 
     JMP  sai_anima_displays            ; na próxima execução já irá para o novo estado
...
; Executa estado VAI_AUMENTAR
     ...
     CMP  R1, R2                        ; nenhuma tecla premida?
     JNZ  sai_anima_displays            ; se ainda houver uma tecla premida, mantém o estado
     MOV  R1, AUMENTA_DISPLAYS          ; se sim, muda para o próximo estado
     MOV  [R0], R1                      ; atualiza a variável de estado 
     JMP  sai_anima_displays            ; na próxima execução já irá para o novo estado
...
; **********************************************************************
; ALTERA_DISPLAYS - Aumenta ou diminui o valor mostrado nos displays,
;                   se o atraso já tiver chegado ao fim.
;                   Atualiza o display e a variável em que o valor é guardado.
; Argumentos: R4 - Número de unidades com que o display deve ser alterado (> 0 ou < 0)
; **********************************************************************
altera_displays:
     PUSH R1
     PUSH R2
     ...
     JNZ  sai_altera_displays
     MOV  R3, contador        ; contador, cujo valor vai ser mostrado nos displays
     MOV  R2, [R3]            ; obtém valor do contador
     ADD  R2, R4              ; altera o valor dos displays
     MOV  [R3], R2            ; atualiza valor do contador
     MOV  R3, DISPLAYS        ; endereço do periférico que liga aos displays
     MOVB [R3], R2            ; mostra o valor do contador nos displays
sai_altera_displays:
     POP  R3
     POP  R2
     POP  R1
