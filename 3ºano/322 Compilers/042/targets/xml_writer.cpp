...
std::string qualifier_to_string(int qualifier) {
...
std::string escape_less_than(const std::string &s) {
  std::string result;
...
void mml::xml_writer::do_declaration_node(mml::declaration_node * const node, int lvl) {
  ...
  os() << std::string(lvl, ' ') << "<" << node->label() << " name='" << node->identifier() << "' qualifier='"
      << qualifier_to_string(node->qualifier()) << "' type='" << escape_less_than(node->type()->to_string()) << "'>" << std::endl;
...
void mml::xml_writer::do_function_call_node(mml::function_call_node * const node, int lvl) {
  ...
  if (node->function()) node->function()->accept(this, lvl + 4); //TODO: function name
...
void mml::xml_writer::do_program_node(mml::program_node * const node, int lvl) {
  openTag(node, lvl);
  openTag("declarations", lvl + 2);
  if (node->file_declarations()) node->file_declarations()->accept(this, lvl + 4);
  closeTag("declarations", lvl + 2);
...
void mml::xml_writer::do_print_node(mml::print_node * const node, int lvl) {
  ASSERT_SAFE_EXPRESSIONS;
  os() << std::string(lvl, ' ') << "<" << node->label() << " newline='" << bool_to_string(node->is_newline()) << "'>" << std::endl;
